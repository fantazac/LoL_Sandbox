...
public class ProjectileUnitTargeted : Projectile
{
    ...
    public void ShootProjectile(Unit target, float speed, bool isACriticalStrike = false, bool willMiss = false)
    {
        if (target != null)
        {
            affectedTeams = AffectedTeams.GetAllyTeam(target.Team);
            this.target = target;
            this.speed = speed;
