...
public class CharacterMouseManager : MonoBehaviour
{
    ...
    private void PressedRightClick(Vector3 mousePosition)
    {
        ...
        {
            character.CharacterMovementManager.PrepareMovementTowardsTarget(HoveredUnit);
        }
        else if (closestEnemyUnit != null)
        {
            character.CharacterMovementManager.PrepareMovementTowardsTarget(closestEnemyUnit);
        }
        else if (hitTerrain)
        {
